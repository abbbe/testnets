...
case $1 in
	...
	run)
		mkdir -p $DATADIR
		docker run -d --net mynet --ip 172.18.0.16 --name $NAME -p 127.0.0.1:$UIPORT:$UIPORT -p 127.0.0.1:$RPCPORT:$RPCPORT -p 127.0.0.1:$WSPORT:$WSPORT \
			...
			parity/parity:stable-release --ui-interface all --jsonrpc-interface all --ws-interface all --ws-origins http://localhost:$UIPORT \
				--ports-shift $PORTSHIFT --chain dev
		;;
